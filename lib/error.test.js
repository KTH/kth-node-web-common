...
const isProd = true
const lang = 'sv'
const mockFormat = formatHandlers => {
  ...
  describe('text/html', () => {
    ...
    describe('handlebar template', () => {
      ...
      it('render template if all required handlebar helpers are registered', () => {
        ...
        expect(res.send).toHaveBeenCalledWith(mockErrorPage)
      })
      it('compile template from string', () => {
        errorHandler.renderErrorPage(res, req, 444, mockI18n, isProd, lang, err)

        expect(handlebars.compile).toHaveBeenCalledWith(expect.stringContaining('<title>{{headTitle}}</title>'))
      })
      it('render template with correct statusCode', () => {
        errorHandler.renderErrorPage(res, req, 456, mockI18n, isProd, lang, err)

        expect(mockTemplate).toHaveBeenCalledWith(expect.objectContaining({ statusCode: 456 }))
      })
      it('render template with translated 404 headTitle', () => {
        errorHandler.renderErrorPage(res, req, 404, mockI18n, isProd, lang, err)

        expect(mockTemplate).toHaveBeenCalledWith(expect.objectContaining({ headTitle: 'error_404_head_title' }))
      })
      it('render template with translated 403 headTitle', () => {
        errorHandler.renderErrorPage(res, req, 403, mockI18n, isProd, lang, err)

        expect(mockTemplate).toHaveBeenCalledWith(expect.objectContaining({ headTitle: 'error_403_head_title' }))
      })
      it('render template with translated 500 headTitle for unknown statuscode', () => {
        ...
        expect(mockTemplate).toHaveBeenCalledWith(expect.objectContaining({ headTitle: 'error_500_head_title' }))
      })
      ...
      it('hides error stack if "isProd = true"', () => {
        ...
        expect(mockTemplate).toHaveBeenCalledWith(expect.objectContaining({ error: {} }))
      })
      ...
      it('shows error stack if "isProd = false"', () => {
        ...
        expect(mockTemplate).toHaveBeenCalledWith(expect.objectContaining({ error: testError }))
      })
    })
    describe('Fallback', () => {
